param(
    [string]$OutPath = "D:\FPGA\ku5P\ad9173-x\output\figures\dds_jesd_flow_editable_v5.vsdx",
    [string]$PngPath = "D:\FPGA\ku5P\ad9173-x\output\figures\dds_jesd_flow_editable_v5.png",
    [switch]$NoPreview
)
...
try {
    ...
    Region $page 3.95 3.1 3.90 5.45 $purple $fp | Out-Null
    Region $page 6.95 3.25 2.05 5.05 $green $fg | Out-Null
    Region $page 8.65 3.25 1.4 5.05 $orange $fo | Out-Null
    Region $page 10.30 3.25 1.70 5.05 $red $fr | Out-Null
    Region $page 12.00 3.25 1.20 5.05 $red $fr | Out-Null
    Region $page 14.00 3.25 2.00 5.05 $red $fr | Out-Null

    Label $page 1.05 5.65 1.55 0.22 "频率配置（软件侧）" $blue 8.0 $true | Out-Null
    Label $page 3.95 5.65 3.35 0.22 "相位累加与并行相位生成（pattern_gen_256.v）" $purple 7.0 $true | Out-Null
    Label $page 6.95 5.65 1.65 0.30 "4 路 DDS 封装模块`n(dds48_phase_to_sine_quad.v)" $green 6.7 $true | Out-Null
    Label $page 8.65 5.65 1.1 0.30 "幅度控制`n(pattern_gen_256.v)" $orange 6.5 $true | Out-Null
    Label $page 10.30 5.65 1.35 0.30 "四通道并行打包`n(pattern_gen_256.v)" $red 6.4 $true | Out-Null
    Label $page 12.00 5.65 0.9 0.30 "JESD204C`nTX IP" $red 7.0 $true | Out-Null
    Label $page 14.00 5.65 1.2 0.22 "AD9173 DAC" $red 7.5 $true | Out-Null
    ...
    for ($i=0; $i -lt 4; $i++) {
        ...
        Label $page 5.55 $phaseY[$i] 0.38 0.14 ("phase_{0}" -f $i) $black 4.6 $false | Out-Null
    }
    ...
    for ($i=0; $i -lt 4; $i++) {
        Box $page 6.25 $rows[$i] 0.55 0.42 "相位高`n16 bit`n[47:32]" $green $white 4.6 $true | Out-Null
        Box $page 7.15 $rows[$i] 1.02 0.48 "Xilinx DDS Compiler IP`ndds_phase_to_sine`n相位 → 正弦" $green $white 4.45 $true | Out-Null
        Line $page @(5.72,$phaseY[$i],5.9,$phaseY[$i],5.9,$rows[$i],5.9,$rows[$i]) $false
        Line $page @(5.88,$rows[$i],5.98,$rows[$i]) $true
        Line $page @(6.53,$rows[$i],6.64,$rows[$i]) $true
        Label $page 7.88 ($rows[$i]+0.11) 0.38 0.14 ("S{0}[15:0]" -f $i) $black 4.5 $false | Out-Null
        Line $page @(7.66,$rows[$i],8.0,$rows[$i]) $true
    }
    Box $page 6.95 1.05 1.70 0.62 "dds_phase_to_sine 为 Xilinx DDS Compiler IP`n使用相位查表（LUT）或 CORDIC 完成正弦量化`n每周期生成 4 路并行 16 bit 正弦采样" $green $white 4.55 $false | Out-Null
    ...
    for ($i=0; $i -lt 4; $i++) {
        Label $page 9.58 ($rows[$i]+0.12) 0.50 0.15 $dac[$i] $black 4.6 $true | Out-Null
        Line $page @(9.32,$rows[$i],9.58,$rows[$i]) $true
        $baseX = 9.85
        for ($j=0; $j -lt 4; $j++) {
            Box $page ($baseX + 0.34*$j) $rows[$i] 0.34 0.34 ("n{0}" -f $j) $red $white 5.4 $true | Out-Null
        }
    }
    Box $page 11.18 3.6 0.68 0.75 "256 bit`n并行数据`n4 Ch ×`n4 采样 ×`n16 bit" $red $white 4.7 $true | Out-Null
    $mergeY = @(3.93,3.72,3.51,3.30)
    for ($i=0; $i -lt 4; $i++) {
        Line $page @(11.04,$rows[$i],11.16,$mergeY[$i]) $false
        Line $page @(11.16,$mergeY[$i],11.18,$mergeY[$i]) $true
    }

    # JESD and DAC.
    Box $page 12.00 4.85 0.9 0.72 "高速串行发送" $red $white 5.8 $true | Out-Null
    Box $page 12.00 3.25 0.95 0.9 "JESD 数据映射`n(tx_mapper.v)`n数据打包与`n字节重排" $red $white 4.8 $true | Out-Null
    Line $page @(12.00,4.49,12.00,3.70) $true $false $red 1.0
    Line $page @(12.00,2.80,12.00,2.10) $true $false $red 1.0
    Line $page @(11.52,3.6,11.53,3.6) $true
    Label $page 12.00 1.92 0.6 0.16 "256 bit" $black 4.8 $true | Out-Null

    Box $page 14.00 4.38 1.05 0.72 "256 bit`n并行输入" $red $white 5.2 $true | Out-Null
    Line $page @(12.00,2.10,13.80,4.00) $true $false $red 0.9
    Sine $page 14.00 3.0 0.95 0.62 $red
    Label $page 14.00 2.35 1.2 0.2 "模拟正弦输出" $red 5.5 $true | Out-Null
