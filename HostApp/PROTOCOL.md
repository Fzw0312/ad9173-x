...
| `0x2c` | reserved | 4 | 预留 |
| `0x30` | rf_atten_code | 1 | PE43711 衰减码，0.25 dB/LSB，范围 0..127 |
| `0x31` | output_path_sel | 1 | B9 输出选通，0=RF/DAC0，1=低频/DAC1 |
...
GUI 只有 CH1/CH2 两个通道时，HostApp 会把 CH1 映射到内部 DAC0、CH2 映射到内部 DAC2；内部 DAC1/DAC3 静音。

JSON 配置中的 `output_mode` 当前支持：

| 值 | 说明 |
| --- | --- |
| `nco_only` | 保持原有 VIO 校准流程，HostApp 通过 Vivado VIO 写 AD9173 片内 NCO 幅度、FTW、PE43711 和输出通路。 |
...
- 收到有效 `K5DC` CONFIG 后更新 48 位 DDS FTW、幅度、PE43711 衰减码和 B9 输出选通。flags bit1=1 时通过 AD9173 runtime SPI 切到片内 NCO-only；flags bit2=1 时保持 RAM 播放状态，避免重复/延迟 CONFIG 把输出退回默认 DDS 正弦。
