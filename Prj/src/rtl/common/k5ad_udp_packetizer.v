...
module k5ad_udp_packetizer #(
    ...
    localparam [2:0] ST_CRC_PAY   = 3'd2;
    localparam [2:0] ST_PREAMBLE  = 3'd3;
    localparam [2:0] ST_FRAME     = 3'd4;
    ...
    wire [15:0] total_samples16 = TOTAL_SAMPLES[15:0];
    wire [15:0] data_chunk_count16 = DATA_CHUNK_COUNT[15:0];

    ipv4_checksum u_ip_checksum (
        .total_length  (ip_len),
        .identification(sequence[15:0]),
        .src_ip        (SRC_IP),
    ...
    always @* begin
        payload_len = K5AD_DATA_HDR_LEN + {chunk_sample_count, 1'b0};
    ...
    always @* begin
        ...
        if (payload_crc_index < K5AD_DATA_HDR_LEN) begin
    ...
    always @* begin
        ...
        if (byte_index < 16'd6) begin
            ...
            case (byte_index - 16'd14)
                ...
                16'd4:  frame_byte = sequence[15:8];
                16'd5:  frame_byte = sequence[7:0];
        ...
        if (rst) begin
            ...
            case (state)
                ...
                ST_LOAD: begin
                    ...
                    payload_crc_index <= 16'd0;
                    payload_crc_state <= 32'hFFFF_FFFF;
                    ram_rd_byte_addr  <= chunk_sample_offset[13:0] * 14'd2;
                    state             <= ST_CRC_PAY;
                end
                ...
                ST_CRC_PAY: begin
                    ...
                    if (payload_crc_index == payload_len - 1'b1) begin
                        ...
                        payload_crc_index <= payload_crc_index + 1'b1;
                    end
                end
                ...
                ST_FRAME: begin
                    ...
                    if (byte_index >= DATA_SAMPLE_BASE) begin
                        ram_rd_byte_addr <= ram_rd_byte_addr + 1'b1;
                    end
                ...
                ST_IFG: begin
                    if (ifg_count < 8'd12) begin
                    ...
                    end else if (chunk_index + 1'b1 < data_chunk_count16) begin
                        ...
                        payload_crc_index   <= 16'd0;
                        payload_crc_state   <= 32'hFFFF_FFFF;
                        ram_rd_byte_addr    <= (chunk_sample_offset + chunk_sample_count) * 14'd2;
                        state               <= ST_CRC_PAY;
                    end else begin
                        sequence <= sequence + 1'b1;
                        busy     <= 1'b0;
    ...
    function [7:0] k5ad_header_byte;
        ...
        begin
            ...
            case (idx)
                ...
                16'd8:  k5ad_header_byte = sequence_i[7:0];
                16'd9:  k5ad_header_byte = sequence_i[15:8];
                16'd10: k5ad_header_byte = sequence_i[23:16];
                16'd11: k5ad_header_byte = sequence_i[31:24];
    ...
    function [7:0] adc_data_payload_header_byte;
        ...
        input [15:0] sample_offset_i;
        input [15:0] sample_count_i;
        input [15:0] total_samples_i;
        begin
